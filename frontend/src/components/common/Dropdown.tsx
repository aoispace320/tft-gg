    ...
    <div className={classNames('relative', className)} ref={ref}>
      ...
      <button
        ...
        className={classNames(
          'flex h-9 w-full items-center justify-between gap-2 rounded-btn border border-line/60 bg-bg-elevated px-3 text-sm text-text-primary transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-brand/50',
          disabled ? 'cursor-not-allowed opacity-50' : 'hover:border-brand',
        )}
      >
            ...
            <li key={opt.value}>
              <button
                ...
                className={classNames(
                  ...
                  opt.value === value ? 'text-brand' : 'text-text-primary',
                )}
              >
                {opt.label}
              </button>
